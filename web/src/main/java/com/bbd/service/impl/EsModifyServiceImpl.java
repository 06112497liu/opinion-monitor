...
public class EsModifyServiceImpl implements EsModifyService {
    ...
    public void transferOpinion(UserInfo operator, Long opOwnerId, TransferParam param) throws IOException, ExecutionException, InterruptedException {
        ...
        request.doc(XContentFactory.jsonBuilder().startObject().field(opStatusField, 1).field(opOwnerField, opOwnerId).field(operatorsField, newArr).field(transferTypeField, param.getTransferType())
            .endObject());
    ...
    public void recordTransfer(OpinionOpRecordVO recordVO) {
        ...
        bulkRequest.add(client.prepareIndex(EsConstant.IDX_OPINION, EsConstant.OPINION_OP_RECORD_TYPE).setSource(JsonUtil.fromJson(recordVO), XContentType.JSON));
        // 插入数据
        bulkRequest.get();
    }
}
